...
ConfigurableProParameters.EnvironmentNumber.value = 50;
ConfigurableProParameters.HeightSeverity.value    = 7.0;
ConfigurableProParameters.WidthSeverity.value     = 1.0;
ConfigurableProParameters.AngleSeverity.value      = pi/9;
ConfigurableProParameters.TauSeverity.value        = 0.2;
ConfigurableProParameters.EtaSeverity.value        = 10.0;

% Generator uses CsvRandom('numbers10f.csv') internally, matching
% dynamic-optimization-kotlin's benchmark.gmpb.comp.GMPB default.
Problem = BenchmarkGenerator_GMPB(BenchmarkName, ConfigurableProParameters);

fprintf('GMPB initialized\n');
fprintf('Max evals: %d\n', Problem.MaxEvals);
...
% Kotlin: val random = CsvRandom("numbers10f.csv")
randomX = CsvRandom('numbers10f.csv');
