...
%*****************************************************************************************
function [fitnesses,Problem,E_bbc,E_o,T_r,CurrentError,VisualizationInfo,Iteration] = main_DSPSO(VisualizationOverOptimization,PeakNumber,ChangeFrequency,SampleInterval,Dimension,ShiftSeverity,EnvironmentNumber,RunNumber,BenchmarkName)
...
for RunCounter=1 : RunNumber
    ...
    [Optimizer.pop,Problem] = SubPopulationGenerator_DSPSO(Optimizer.Dimension,Optimizer.MinCoordinate,Optimizer.MaxCoordinate,Optimizer.PopulationSize,Problem);
    VisualizationFlag=0;
    Iteration=0;
    if VisualizationOverOptimization==1
    ...
    while 1
        ...
        %% Optimization
        [Optimizer,Problem] = IterativeComponents_DSPSO(Optimizer,Problem);
        if Problem.RecentChange == 1%When an environmental change has happened
            Problem.RecentChange = 0;
            [Optimizer,Problem] = ChangeReaction_DSPSO(Optimizer,Problem);
            VisualizationFlag = 0;
