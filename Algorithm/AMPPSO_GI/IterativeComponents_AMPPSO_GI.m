...
%**************************************************************************************************
function [Optimizer, Problem] = IterativeComponents_AMPPSO_GI(Optimizer,Problem)
...
for ii=1 : Optimizer.SwarmNumber
    ...
    if(Optimizer.pop(ii).IsStagnated == 0)
        for tries = 1:2
            randval = randn();
            vals = Optimizer.pop(ii).PbestValue(:);
            [~,index] = sort(-vals); 
            for gg = 1:Problem.Dimension
                ...
                newpos1 = Optimizer.pop(ii).PbestPosition(index(1),gg) + randn()*Optimizer.pop(ii).CurrentRadius;
                Optimizer.pop(ii).X(Optimizer.pop(ii).GbestID,gg) = newpos1 + randval*(Optimizer.pop(ii).PbestPosition(index(2),gg) - newpos1);
    ...
    elseif(Optimizer.pop(ii).IsStagnated == 1)
        for tries = 1:1
            ...
            for gg = 1:Problem.Dimension
                newpos1 = Optimizer.pop(ii).PbestPosition(index(1),gg) + randn()*Optimizer.pop(ii).CurrentRadius;
                newpos2 = newpos1 + randval*(Optimizer.pop(ii).PbestPosition(index(2),gg) - newpos1);
                ...
                Optimizer.pop(ii).PbestPosition(Optimizer.pop(ii).GbestID,gg) = newpos2;            
...
if(NonStagnatedRadius < 0.005 * Optimizer.domainS)
    ...
    if(NumAddParticles > 0)
        ...
        [Swarms,remainIndivs,Problem] = SubPopulationGenerator_AMPPSO_GI(Optimizer.Dimension,AddParticles,Problem);
