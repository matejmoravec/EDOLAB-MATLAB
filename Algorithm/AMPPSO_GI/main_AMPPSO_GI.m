...
%**************************************************************************************************
function [Problem,Results,CurrentError,VisualizationInfo,Iteration,Moravec] = main_AMPPSO_GI(VisualizationOverOptimization, RunNumber, BenchmarkName, ConfigurableProParameters, ConfigurableAlgParameters, progressInfo, Moravec)
...
for RunCounter=1 : RunNumber
    ...
    [Optimizer.pop,Optimizer.remainIndivs,Problem] = SubPopulationGenerator_AMPPSO_GI(Optimizer.Dimension,InitSwarm,Problem);
    ...
    while 1
        ...
        %% Optimization
        [Optimizer,Problem] = IterativeComponents_AMPPSO_GI(Optimizer,Problem);
        if Problem.RecentChange == 1%When an environmental change has happened
            Problem.RecentChange = 0;
            [Optimizer,Problem] = ChangeReaction_AMPPSO_GI(Optimizer,Problem);
            VisualizationFlag = 0;
