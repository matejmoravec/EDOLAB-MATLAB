...
%*****************************************************************************************
function [fitnesses,Problem,E_bbc,E_o,T_r,CurrentError,VisualizationInfo,Iteration] = main_ImQSO(VisualizationOverOptimization,PeakNumber,ChangeFrequency,SampleInterval,Dimension,ShiftSeverity,EnvironmentNumber,RunNumber,BenchmarkName)
...
for RunCounter=1 : RunNumber
    ...
    while 1
        ...
        %% Optimization
        [Optimizer,Problem] = IterativeComponents_ImQSO(Optimizer,Problem);
        if Problem.RecentChange == 1%When an environmental change has happened
            Problem.RecentChange = 0;
            [Optimizer,Problem] = ChangeReaction_ImQSO(Optimizer,Problem);
            VisualizationFlag = 0;
