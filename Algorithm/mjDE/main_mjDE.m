...
%*****************************************************************************************
function [fitnesses,Problem,E_bbc,E_o,T_r,CurrentError,VisualizationInfo,Iteration] = main_mjDE(VisualizationOverOptimization,PeakNumber,ChangeFrequency,SampleInterval,Dimension,ShiftSeverity,EnvironmentNumber,RunNumber,BenchmarkName)
...
for RunCounter=1 : RunNumber
    ...
    Optimizer.FreeSwarmID = 1;%Index of the free swarm    Optimizer.ExclusionLimit = 0.5 * ((Optimizer.MaxCoordinate-Optimizer.MinCoordinate) / ((Optimizer.SwarmNumber) ^ (1 / Optimizer.Dimension)));
    ...
    [Optimizer.pop,Problem] = SubPopulationGenerator_mjDE(Optimizer.Dimension,Optimizer.MinCoordinate,Optimizer.MaxCoordinate,Optimizer.PopulationSize,Problem);
    VisualizationFlag=0;
    Iteration=0;
    if VisualizationOverOptimization==1
    ...
    while 1  
        ...
        %% Optimization
        [Optimizer,Problem] = IterativeComponents_mjDE(Optimizer,Problem);
        if Problem.RecentChange == 1%When an environmental change has happened
            Problem.RecentChange = 0;
            [Optimizer,Problem] = ChangeReaction_mjDE(Optimizer,Problem);
            VisualizationFlag = 0;
