...
%**************************************************************************************************
function [Problem,Results,CurrentError,VisualizationInfo,Iteration,Moravec] = main_AMPDE(VisualizationOverOptimization, RunNumber, BenchmarkName, ConfigurableProParameters, ConfigurableAlgParameters, progressInfo, Moravec)
...
for RunCounter=1 : RunNumber
    ...
    Problem = BenchmarkGenerator(BenchmarkName, ConfigurableProParameters);        rng('shuffle');%Set a random seed for the optimizer
    ...
    [Optimizer.pop,Optimizer.remainIndivs,Problem] = SubPopulationGenerator_AMPDE(Optimizer.Dimension,Optimizer.MinCoordinate,Optimizer.MaxCoordinate,InitSwarm,Problem);
    ...
    while 1
        ...
        %% Optimization
        [Optimizer,Problem] = IterativeComponents_AMPDE(Optimizer,Problem);
        if Problem.RecentChange == 1%When an environmental change has happened
            Problem.RecentChange = 0;
            [Optimizer,Problem] = ChangeReaction_AMPDE(Optimizer,Problem);
            VisualizationFlag = 0;
