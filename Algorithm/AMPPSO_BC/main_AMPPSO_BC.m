...
%**************************************************************************************************
function [Problem,Results,CurrentError,VisualizationInfo,Iteration,Moravec] = main_AMPPSO_BC(VisualizationOverOptimization, RunNumber, BenchmarkName, ConfigurableProParameters, ConfigurableAlgParameters, progressInfo, Moravec)
...
for RunCounter=1 : RunNumber
    ...
    Optimizer.ShiftSeverity = Problem.ShiftSeverity;%initial shift severity
    ...
    [Optimizer.pop,Optimizer.remainIndivs,Problem] = SubPopulationGenerator_AMPPSO_BC(Optimizer.Dimension,InitSwarm,Problem);
    ...
    while 1
        ...
        %% Optimization
        [Optimizer,Problem] = IterativeComponents_AMPPSO_BC(Optimizer,Problem);
        if Problem.RecentChange == 1%When an environmental change has happened
            Problem.RecentChange = 0;
            [Optimizer,Problem] = ChangeReaction_AMPPSO_BC(Optimizer,Problem);
            VisualizationFlag = 0;
