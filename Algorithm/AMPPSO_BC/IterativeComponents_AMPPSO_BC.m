...
%**************************************************************************************************
function [Optimizer, Problem] = IterativeComponents_AMPPSO_BC(Optimizer,Problem)
...
for ii=1 : Optimizer.SwarmNumber
  ...
  %Brownian move
    if(Optimizer.pop(ii).IsStagnated == 0)
        % 保存原始位置
        original_position = Optimizer.pop(ii).X(Optimizer.pop(ii).GbestID,:);
        
        for tries = 1:2  % 保持原来的尝试次数
            % 使用轻微放大的搜索半径
            search_radius = Optimizer.pop(ii).CurrentRadius * (1 + 0.1 * tries);
            
            for gg = 1:Problem.Dimension
                Optimizer.pop(ii).X(Optimizer.pop(ii).GbestID,gg) = Optimizer.pop(ii).PbestPosition(Optimizer.pop(ii).GbestID,gg) + randn() * search_radius;
            ...
            if(BestResult > Optimizer.pop(ii).PbestValue(Optimizer.pop(ii).GbestID))
                ...
                break;  % 找到更好的解后停止
            elseif tries < 2  % 如果这不是最后一次尝试
                % 重置位置，准备下一次尝试
                Optimizer.pop(ii).X(Optimizer.pop(ii).GbestID,:) = original_position;
            end
    ...
    elseif(Optimizer.pop(ii).IsStagnated == 1)
        %Cauchy move
        for gg = 1:Problem.Dimension
            % 修改：根据环境变化次数动态调整柯西分布参数
            scale_factor = max(0.1, min(0.5, Problem.Environmentcounter / 20)) * (Optimizer.MaxCoordinate - Optimizer.MinCoordinate);
            pd = makedist('tLocationScale','mu',Optimizer.pop(ii).PbestPosition(Optimizer.pop(ii).GbestID,gg),'sigma',scale_factor,'nu',1);
...
if(NonStagnatedRadius < 0.005 * Optimizer.domainS)
    ...
    if(NumAddParticles > 0)
        ...
        [Swarms,remainIndivs,Problem] = SubPopulationGenerator_AMPPSO_BC(Optimizer.Dimension,AddParticles,Problem);
