...
%**************************************************************************************************
function [Problem,Results,CurrentError,VisualizationInfo,Iteration,Moravec] = main_DynDE(VisualizationOverOptimization, RunNumber, BenchmarkName, ConfigurableProParameters, ConfigurableAlgParameters, progressInfo, Moravec)
...
for RunCounter=1 : RunNumber
    ...
    Optimizer.ExclusionLimit = 15.5;
    Optimizer.HiberStep = 0.5;
    Optimizer.OverlapDegree = 0.1;
    Optimizer.FirstUpdate = 1;
    %Choose Diversity StrategyFlag
    % diversityMap = containers.Map(...
    %     {'Brownian Individuals', 'Quantum Individuals', 'Normal Individuals'}, ...
    %     [1, 2, 3] ...
    % );
    % Optimizer.DiversityFlag = diversityMap(ConfigurableAlgParameters.DiversityFlag.value);    % 1: Brownian Individuals    
                                                                                                % 2: Quantum Individuals
    Optimizer.DiversityFlag = 1;    
    %Random CR/F or Not
    randomMap = containers.Map(...
        {'Random CR/F', 'Fixed CR/F'}, ...
        [1, 2] ...
    );
    Optimizer.RandomFlag = randomMap(ConfigurableAlgParameters.RandomFlag.value);       % 1: Random CR/F             
                                                                                      % 2: Fixed CR/F
    %Choose DE Strategy
    strategyMap = containers.Map(...
        {'Random Strategy', 'DE/RAND/1', 'DE/RAND/2', 'DE/BEST/1', 'DE/BEST/2', 'DE/RAND-TO-BEST/1', 'DE/CURRENT-TO-RAND/1', 'DE/CURRENT-TO-BEST/1'}, ...
        [0, 1, 2, 3, 4, 5, 6, 7] ...
    );
    ...
    Optimizer.StrategyFlag = strategyMap(ConfigurableAlgParameters.StrategyFlag.value); 
    [Optimizer.pop,Problem] = SubPopulationGenerator_DynDE(Optimizer.Dimension,Optimizer.MinCoordinate,Optimizer.MaxCoordinate,Optimizer.SwarmNumber,Optimizer.IndivSize,Problem,Optimizer.DiversityFlag,Optimizer.RandomFlag,Optimizer.StrategyFlag);
    ...
    while 1
        ...
        %% Optimization
        [Optimizer,Problem] = IterativeComponents_DynDE(Optimizer,Problem);
        if Problem.RecentChange == 1%When an environmental change has happened
            Problem.RecentChange = 0;
            [Optimizer,Problem] = ChangeReaction_DynDE(Optimizer,Problem);
            VisualizationFlag = 0;
