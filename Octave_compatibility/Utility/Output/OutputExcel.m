...
function OutputExcel(AlgorithmName,BenchmarkName,ChangeFrequency,Dimension,PeakNumber,ShiftSeverity,RunNumber,EnvironmentNumber,E_o,E_bbc,T_r,pathout)
   ...
   % hSheets=hExcel.ActiveWorkBook.Sheets.Item(1);
    %hSheets.Range('A1:D1').ColumnWidth=36;  % Width
   ...
   % hSheets.Range('A1:A1').Font.underline  = 2; % Underline
    %hSheets.Range('A1:D1').MergeCells = 1;
    %hSheets.Range('A2:D2').MergeCells = 1;
   ...
   % hSheets.Range('A4:A4').Font.color = 255; % Font color
    ...
    %hSheets.Range('A4:D4').MergeCells = 1;
    ...
    %hSheets.Range('A12:D12').MergeCells = 1; % Merge cells
   ...
   % hSheets.Range('A5:A10').Font.size = 12; % Font size
    ...
    %    hSheets.Range(['B',num2str(i),':','D',num2str(i)]).MergeCells = 1;
    %end

   % hSheets.Range('A13:D13').Font.bold = 2;  % Font blod
   % hSheets.Range('A13:D13').Font.size = 14; % Font size
    %hSheets.Range('A13:D13').Font.color = 16711680; % Font color
   ...
   % hSheets.Range(['A14',':A',num2str(14 + RunNumber - 1)]).Font.Italic = 2; % Italic
    ...
    %hSheets.Range('B1:D1').Font.bold = 2;  % Font blod
   % hSheets.Range('B1:D1').Font.size = 14; % Font size
